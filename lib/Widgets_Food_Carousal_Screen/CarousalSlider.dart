...
class CarousalSliderWidget extends StatelessWidget {
  ...
  const CarousalSliderWidget({Key key, this.currentPostedDish, this.setIndex}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    final List<String> imgList = currentPostedDish.map((e) =>e.dishPictureURI).toList();
        ...
        .map((item) => Container(
              child: Container(
                ...
                child: ClipRRect(
                    ...
                    child: Stack(
                      children: <Widget>[
                        cZeroStr(item)?Image.network(item):Image.asset('assets/images/carouselfirst.png', fit: BoxFit.cover, width: 1000.0),
                      ],
                    )),
              ),
            ))
